...
<title>NexShop — Messages contact</title>
...
:root{
  ...
  --border:rgba(255,255,255,.07);--orange:#FF6B35;--orange2:#FF8C5A;
  --blue:#1E90FF;--white:#FFFFFF;--text:#F0F0F0;--muted:#777;--muted2:#444;
  --success:#22C55E;--danger:#EF4444;--warning:#F59E0B;
  --radius:14px;--radius-xs:7px;
}
...
.admin-badge{background:rgba(255,107,53,.12);border:1px solid rgba(255,107,53,.25);color:var(--orange);font-size:10px;font-weight:700;padding:3px 10px;border-radius:50px;font-family:'Space Grotesk',sans-serif}
.nav-right{margin-left:auto;display:flex;align-items:center;gap:8px}
.nav-logout-btn{display:inline-flex;align-items:center;gap:7px;padding:8px 14px;border-radius:var(--radius-xs);border:1px solid var(--border);background:var(--bg3);color:var(--muted);font-size:12px;font-weight:700;cursor:pointer;font-family:'Space Grotesk',sans-serif}
...
.sb-item{display:flex;align-items:center;gap:9px;padding:9px 10px;border-radius:var(--radius-xs);color:var(--muted);font-size:13px;font-weight:500;border:1px solid transparent}
...
.table th{padding:11px 16px;text-align:left;font-size:11px;font-weight:700;color:var(--muted);letter-spacing:.07em;text-transform:uppercase;border-bottom:1px solid var(--border);background:var(--bg3);font-family:'Space Grotesk',sans-serif}
...
.tbl-btn{width:28px;height:28px;border-radius:var(--radius-xs);background:var(--bg3);border:1px solid var(--border);color:var(--muted);cursor:pointer;display:flex;align-items:center;justify-content:center}
...
@media(max-width:768px){.sidebar{display:none}.main{padding:18px 14px}}
</style>
...
<nav class="navbar">
  ...
  <div class="nav-right">
    <span style="font-family:'Space Grotesk',sans-serif;font-size:13px;font-weight:600">{{ Auth::user()->nom }}</span>
...
<div class="layout">
  <aside class="sidebar">
    ...
    <a href="{{ route('admin.kyc.index') }}" class="sb-item"><i class="fa-solid fa-id-card-clip"></i> Inscriptions vendeurs @if($__pendingKycVendeurs)<span style="margin-left:auto;font-size:10px;font-weight:800;color:var(--orange)">{{ $__pendingKycVendeurs }}</span>@endif</a>
  ...
  <main class="main">
    @yield('contact-messages-content')
  </main>
</div>
</body>
</html>
